...
def main() -> None:
    ...
    parser.add_argument('--vehicle-workers', metavar='NUM_THREADS', required=False, type=int, choices=range(1, 9), default=default.NUM_VEHICLE_WORKERS, help=f'Quantidade de threads responsáveis pelo processamento dos quadros do vídeo (default = {default.NUM_VEHICLE_WORKERS}).')
    ...
    parser.add_argument('--vehicle-queue-size', metavar='SIZE', required=False, type=int, default=default.VEHICLE_QUEUE_SIZE, help=f'Tamanho do buffer que mantém as imagens de veículos não processadas (default = {default.VEHICLE_QUEUE_SIZE}).')
